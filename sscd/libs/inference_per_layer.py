...
#     def __init__(self, model, metadata_keys, save_path='./intermediate_outputs'):
#         super().__init__()
#         self.model = model
...
#     def forward(self, x):
#         layer_outputs = []
#         for idx, block in enumerate(self.model.backbone.blocks):
#             x = block(x)
#             layer_outputs.append(x)
        
#         # 블럭별 저장
#         for idx, output in enumerate(layer_outputs):
#             save_path = os.path.join(self.save_path, f'layer_{idx}.pth')
#             torch.save(output.cpu().detach(), save_path)
    ...
    def inference(cls, args, dataset, base_name="predictions"):
        if args.features:
            ...
            if args.checkpoint:
                ...
                new_state_dict = OrderedDict((k.replace("model.", ""), v) for k, v in state['state_dict'].items())
                ...
                # 230901
                # pl_model = tx.Extractor(pl_model, ["blocks.0", "blocks.1", "blocks.2", "blocks.3", "blocks.4","blocks.5","blocks.6","blocks.7","blocks.8","blocks.9","blocks.10","blocks.11"])
